...
def start_message_gen(user_name_for_start, user_id):
...
def help_message_gen(user_name_for_start, user_id):
    client = gemini_client
    response = client.models.generate_content(
        model="gemini-2.0-flash",
        config=types.GenerateContentConfig(
...
def feedback_start_message_gen(user_name_for_start, user_id):
    client = gemini_client
    response = client.models.generate_content(
        model="gemini-2.0-flash",
        config=types.GenerateContentConfig(
    ...
    return response.text
# Функция для генерации сообщения при вводе команды /feedback
def feedback_end_message_gen(user_name_for_start,feedback_type,feedback_message,user_id):
    client = gemini_client
    response = client.models.generate_content(
        model="gemini-2.0-flash",
        config=types.GenerateContentConfig(
